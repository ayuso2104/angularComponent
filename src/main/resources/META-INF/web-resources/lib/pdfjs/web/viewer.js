...
var PDFViewerApplication = {
  ...
  open: function pdfViewOpen(file, args) {
    ...
    parameters.data = window.archivoBlob;
...
function webViewerInitialized() {
  ...
  Promise.all(waitForBeforeOpening).then(function () {
    webViewerOpenFileViaURL(appConfig.dataBlob);
  }).catch(function (reason) {
    PDFViewerApplication.error(mozL10n.get('loading_error', null, 'An error occurred while opening.'), reason);
  });
}
var webViewerOpenFileViaURL;
webViewerOpenFileViaURL = function webViewerOpenFileViaURL(dataBlob) {
    PDFViewerApplication.open(dataBlob);
};
...
function getViewerConfiguration(archivoBlob) {
  return {
    ...
    dataBlob: archivoBlob
  };
}
function webViewerLoad(archivoBlob) {
  var config = getViewerConfiguration(archivoBlob);
...
window.renderPDF = function (archivoBlob) {
    window.archivoBlob = archivoBlob;
    webViewerLoad(archivoBlob);
}
