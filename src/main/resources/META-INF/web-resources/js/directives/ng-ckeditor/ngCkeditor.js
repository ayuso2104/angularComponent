...
(function () {
    ...
    function ckeditorDirective($parse) {
      return {
        ...
        link: function (scope, element, attrs, ctrls) {
        	console.info('CKEDITOR INICIALIZADO');
        	
        	// get needed controllers
          var controller = ctrls[0]; // our own, see below
          var ngModelController = ctrls[1];
          var cadena = '<html><head><title></title></head><body></body></html>';
    ...
    function ckeditorController($scope, $element, $attrs, $parse, $q) {
      ...
      this.onCKEvent('dialogDefinition', function (ev) {
          ...
          dialogDefinition.dialog.on('show', function () {
              ...
              var botones = document
                  .getElementsByClassName('cke_dialog_close_button');

              for (var i = 0; i < botones.length; i++) {
                  botones[i].addEventListener('onmousedown', _closeDialog);
                  botones[i].onmousedown = _closeDialog;
              }

              function _closeDialog(event) {
                  var numBoton = event.button;
